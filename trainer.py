...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        # # tr_loss = torch.tensor(0.0).to(self.args.device)  # 原始写法
        tr_loss = 0.0
        tr_loss_mrc = 0.0
        tr_loss_ig = 0.0
        logging_loss_scalar = 0.0
        logging_loss_mrc_scalar = 0.0
        logging_loss_ig_scalar = 0.0
        model.zero_grad()
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                loss, loss_mrc, loss_ig = self.training_step(model, inputs)
                tr_loss += loss.item()
                tr_loss_mrc += loss_mrc.item()
                tr_loss_ig += loss_ig.item()
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (self.args.logging_steps > 0 and self.global_step % self.args.logging_steps == 0) or (
                        ...
                        tr_loss_mrc_scalar = tr_loss_mrc
                        tr_loss_ig_scalar = tr_loss_ig

                        logs["loss"] = (tr_loss_scalar - logging_loss_scalar) / self.args.logging_steps
                        logs["loss_mrc"] = (tr_loss_mrc_scalar - logging_loss_mrc_scalar) / self.args.logging_steps
                        logs["loss_ig"] = (tr_loss_ig_scalar - logging_loss_ig_scalar) / self.args.logging_steps
                        ...
                        logging_loss_mrc_scalar = tr_loss_mrc_scalar
                        logging_loss_ig_scalar = tr_loss_ig_scalar
        ...
        # return TrainOutput(self.global_step, tr_loss.item() / self.global_step)     # 原始写法
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.n_gpu > 1:
            ...
            loss_mrc = loss_mrc.mean()  # mean() to average on multi-gpu parallel training
            loss_ig = loss_ig.mean()  # mean() to average on multi-gpu parallel training

        if self.args.gradient_accumulation_steps > 1:
            loss = loss / self.args.gradient_accumulation_steps
            loss_mrc = loss_mrc / self.args.gradient_accumulation_steps
            loss_ig = loss_ig / self.args.gradient_accumulation_steps
    ...
    def prediction_loop(
        ...
        eval_losses_mrc: List[float] = []
        eval_losses_ig: List[float] = []
        ...
        false_examples = []
        data_index = 0
        input_features = dataloader.batch_sampler.sampler.data_source.features
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            loss, loss_mrc, loss_ig, logits, labels = self.prediction_step(model, inputs, prediction_loss_only)

            batch_size = inputs[list(inputs.keys())[0]].shape[0]
            samples_count += batch_size
            if loss is not None:
                eval_losses.append(loss * batch_size)
            if loss_mrc is not None:
                eval_losses_mrc.append(loss_mrc * batch_size)
            if loss_ig is not None:
                eval_losses_ig.append(loss_ig * batch_size)
            if logits is not None:
                preds = logits if preds is None else torch.cat((preds, logits), dim=0)
            if labels is not None:
                label_ids = labels if label_ids is None else torch.cat((label_ids, labels), dim=0)

            start_data_index = data_index * 2
            if (start_data_index+batch_size) > len(input_features):
                end_data_index = len(input_features)
            else:
                end_data_index = start_data_index + batch_size

            # 得到错误的题目序号
            data_index = data_index + 1
            judge_input_features = dataloader.batch_sampler.sampler.data_source.features[start_data_index:end_data_index]
            pred_labs = torch.argmax(logits, dim=-1)
            for idx in range(end_data_index-start_data_index):
                if pred_labs[idx] != labels[idx]:
                    false_example = judge_input_features[idx]
                    path = false_example.example_id.split('/')
                    type_id = path[-1].split("\\")
                    quest_type = type_id[0]
                    id = type_id[-1].split(".")[0]
                    false_example = {"question_type": quest_type, "id": id}
                    false_examples.append(false_example)

        save_path = os.path.join(self.args.output_dir, "false_example.json")
        with open(save_path, 'w', encoding="utf-8") as fw:
            json.dump(false_examples, fw, indent=4, ensure_ascii=False)
        ...
        if self.args.local_rank != -1:
            ...
            if eval_losses_mrc is not None:
                eval_losses_mrc = xm.mesh_reduce("eval_losses", torch.tensor(eval_losses_mrc), torch.cat).tolist()
            if eval_losses_ig is not None:
                eval_losses_ig = xm.mesh_reduce("eval_losses", torch.tensor(eval_losses_ig), torch.cat).tolist()
        ...
        if len(eval_losses_mrc) > 0:
            metrics["eval_loss_mrc"] = np.sum(eval_losses_mrc) / samples_count
        if len(eval_losses_ig) > 0:
            metrics["eval_loss_ig"] = np.sum(eval_losses_ig) / samples_count
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            outputs = model(**inputs)
            if has_labels:
                loss, loss_mrc, loss_ig, logits = outputs[:4]
                loss = loss.mean().item()
                loss_mrc = loss_mrc.mean().item()
                loss_ig = loss_ig.mean().item()
        ...
        return (loss, loss_mrc, loss_ig, logits.detach(), labels)
